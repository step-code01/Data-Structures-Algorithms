...
/*my method doesn't work for non-Y cases, hence the common method of measuring distances from end is correct*/
#include <iostream>
using namespace std;
class sll;
class node{
...
void find_intersection(sll y_sll,sll y_branch){ // by value is intentional i don't want any changes; const and by ref bhi can?
    ...
    node* lastCommon = nullptr;
    while (walker1!=nullptr && walker2!=nullptr) {
        if (walker1 == walker2) {
            lastCommon = walker1; // still same, remember it
            walker1 = walker1->next;
            walker2 = walker2->next;
        }
        else {
            cout << "Intersection node: " << lastCommon->data << endl;
            break;
        }
    }
}
...
int main(){ 
    ...
    cout <<"Enter the element you want to make the branch at:";
    int x;cin >> x;
    y_sll.y_maker(x,y_branch);
